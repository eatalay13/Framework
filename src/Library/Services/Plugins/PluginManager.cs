...
namespace Services.Plugins
{
    ...
    public partial class PluginManager<TPlugin> : IPluginManager<TPlugin> where TPlugin : class, IPlugin
    {
        ...
        /// <returns>Key</returns>
        protected virtual string GetKeyAsync(string systemName = null)
        {
            return $"lib-emrah-{systemName}";
        }
        ...
        protected virtual async Task<TPlugin> LoadPrimaryPluginAsync(string systemName)
        {
            //try to get a plugin by system name or return the first loaded one (it's necessary to have a primary active plugin)
            var plugin = await LoadPluginBySystemNameAsync(systemName)
                         ?? (await LoadAllPluginsAsync()).FirstOrDefault();
        ...
        public virtual async Task<IList<TPlugin>> LoadAllPluginsAsync()
        {
            //get plugins and put them into the dictionary to avoid further loading
            var key = GetKeyAsync();
            if (!_plugins.ContainsKey(key))
                _plugins.Add(key, await _pluginService.GetPluginsAsync<TPlugin>());
        ...
        /// <param name="storeId">Filter by store; pass 0 to load all plugins</param>
        /// <returns>Plugin</returns>
        public virtual async Task<TPlugin> LoadPluginBySystemNameAsync(string systemName)
        {
            if (string.IsNullOrEmpty(systemName))
                return null;

            //try to get already loaded plugin
            var key = GetKeyAsync(systemName);
            if (_plugins.ContainsKey(key))
                return _plugins[key].FirstOrDefault();

            //or get it from list of all loaded plugins or load it for the first time
            var pluginBySystemName = _plugins.TryGetValue(GetKeyAsync(), out var plugins)
                ...
                ? loadedPlugin
                : (await _pluginService.GetPluginDescriptorBySystemNameAsync<TPlugin>(systemName))?.Instance<TPlugin>();
        ...
        public virtual async Task<IList<TPlugin>> LoadActivePluginsAsync(List<string> systemNames)
        {
            if (systemNames == null)
                return new List<TPlugin>();

            //get loaded plugins according to passed system names
            return (await LoadAllPluginsAsync())
