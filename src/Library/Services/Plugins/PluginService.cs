...
namespace Services.Plugins
{
    ...
    public partial class PluginService : IPluginService
    {
        ...
        public PluginService(IMigrationManager migrationManager,
            ILogger logger,
        ...
        public virtual async Task<PluginDescriptor> GetPluginDescriptorBySystemNameAsync<TPlugin>(string systemName,
            LoadPluginsMode loadMode = LoadPluginsMode.InstalledOnly, string @group = null) where TPlugin : class, IPlugin
        {
            return (await GetPluginDescriptorsAsync<TPlugin>(loadMode, group))
        ...
        public virtual Task<string> GetPluginLogoUrlAsync(PluginDescriptor pluginDescriptor)
        {
            ...
            var logoExtension = LibPluginDefaults.SupportedLogoImageExtensions
                .FirstOrDefault(ext => _fileProvider.FileExists(_fileProvider.Combine(pluginDirectory, $"{LibPluginDefaults.LogoFileName}.{ext}")));
            if (string.IsNullOrWhiteSpace(logoExtension))
                return Task.FromResult<string>(null);

            var storeLocation = _webHelper.GetStoreLocation();
            var logoUrl = $"{storeLocation}{LibPluginDefaults.PathName}/" +
                $"{_fileProvider.GetDirectoryNameOnly(pluginDirectory)}/{LibPluginDefaults.LogoFileName}.{logoExtension}";
            ...
            if (checkDependencies)
            {
                ...
                if (descriptor.DependsOn?.Any() ?? false)
                {
                    ...
                    if (dependsOn.Any())
                    {
                        ...
                        var errorMessage = "PreparePluginToInstallAsync";
        ...
        public virtual async Task PreparePluginToUninstallAsync(string systemName)
        {
            ...
            if (dependentPlugins.Any())
            {
                ...
                if (dependsOn.Any())
                {
                    ...
                    var errorMessage = "PreparePluginToUninstallAsync Hata";
        ...
        public virtual async Task InstallPluginsAsync()
        {
            ...
            foreach (var descriptor in pluginDescriptors.OrderBy(pluginDescriptor => pluginDescriptor.DisplayOrder))
            {
                ...
                {
                    //log error
                    var message = descriptor.SystemName;
