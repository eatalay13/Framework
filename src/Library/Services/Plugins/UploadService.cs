...
namespace Services.Plugins
{
    ...
    public class UploadService : IUploadService
    {
        ...
        public UploadService(ILibFileProvider fileProvider, IThemeProvider themeProvider)
        {
        ...
        protected virtual IList<UploadedItem> GetUploadedItems(string archivePath)
        {
            ...
            var uploadedItemsFileEntry = archive.Entries
                .FirstOrDefault(entry => entry.Name.Equals(LibPluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
        ...
        protected virtual IDescriptor UploadSingleItem(string archivePath)
        {
            ...
            var pluginsDirectory = _fileProvider.MapPath(LibPluginDefaults.Path);

            //get path to the themes directory
            var themesDirectory = string.Empty;
            if (!string.IsNullOrEmpty(LibPluginDefaults.ThemesPath))
                themesDirectory = _fileProvider.MapPath(LibPluginDefaults.ThemesPath);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                if (rootDirectories.Count != 1)
                {
                    throw new Exception("The archive should contain only one root plugin or theme directory. " +
                        ...
                        $"To upload multiple items, the archive should have the '{LibPluginDefaults.UploadedItemsFileName}' file in the root");
                }
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    var isPluginDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{LibPluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                    //or whether it's a theme descriptor
                    var isThemeDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{LibPluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    if (item.Type == UploadedItemType.Plugin)
                        descriptorPath = $"{itemPath}{LibPluginDefaults.DescriptionFileName}";

                    if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(LibPluginDefaults.ThemeDescriptionFileName))
                        descriptorPath = $"{itemPath}{LibPluginDefaults.ThemeDescriptionFileName}";
        ...
        public virtual IList<IDescriptor> UploadPluginsAndThemes(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var tempDirectory = _fileProvider.MapPath(LibPluginDefaults.UploadsTempPath);
        ...
        public virtual void UploadIconsArchive(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var storeIconsPath = _fileProvider.GetAbsolutePath(string.Format(LibCommonDefaults.FaviconAndAppIconsPath));
        ...
        public virtual void UploadFavicon(IFormFile favicon)
        {
            ...
            var storeFaviconPath = _fileProvider.GetAbsolutePath(string.Format(LibCommonDefaults.FaviconAndAppIconsPath));
        ...
        public virtual void UploadLocalePattern(CultureInfo cultureInfo = null)
        {
            ...
            try
            {
                ...
                var ziplocalePatternPath = getPath(LibCommonDefaults.LocalePatternPath, LibCommonDefaults.LocalePatternArchiveName);
                if (!_fileProvider.GetFileExtension(ziplocalePatternPath)?.Equals(".zip", StringComparison.InvariantCultureIgnoreCase) ?? true)
                    throw new Exception($"Archive '{LibCommonDefaults.LocalePatternArchiveName}' to retrieve localization patterns not found.");
                ...
                if (!(checkDirectoryExists(LibCommonDefaults.LocalePatternPath, currentCulture.Name) ||
                    checkDirectoryExists(LibCommonDefaults.LocalePatternPath, currentCulture.TwoLetterISOLanguageName)))
                {
                    var cultureToUse = string.Empty;

                    //3. Unpack the archive into a temporary folder
                    ZipFile.ExtractToDirectory(ziplocalePatternPath, getPath(LibCommonDefaults.LocalePatternPath, tempFolder));

                    //4. Search in the temp unpacked archive a folder with locales by culture
                    var sourcelocalePath = _fileProvider.Combine(getPath(LibCommonDefaults.LocalePatternPath, tempFolder), currentCulture.Name);
                    if (_fileProvider.DirectoryExists(sourcelocalePath))
                    {
                        cultureToUse = currentCulture.Name;
                    }

                    var sourcelocaleISOPath = _fileProvider.Combine(getPath(LibCommonDefaults.LocalePatternPath, tempFolder), currentCulture.TwoLetterISOLanguageName);
                    ...
                    {
                        var destlocalePath = getPath(LibCommonDefaults.LocalePatternPath, cultureToUse);
            ...
            finally
            {
                ...
                if (checkDirectoryExists(LibCommonDefaults.LocalePatternPath, tempFolder))
                    _fileProvider.DeleteDirectory(getPath(LibCommonDefaults.LocalePatternPath, tempFolder));
            }
        }
